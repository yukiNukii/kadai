...
//２．データ登録SQL作成
$stmt = $pdo->prepare("SELECT * FROM gs_cms_table ORDER BY indate DESC LIMIT 3");
$status = $stmt->execute();
...
if($status==false){
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    $view .= '<dt class="news-list--date">'.$result["indate"].'</dt>';
...
<!--
    <textarea name="naiyou" id="editor1" cols="10" rows="10">
        this is my textarea to be replace with CHEditor.
    </textarea>
-->
